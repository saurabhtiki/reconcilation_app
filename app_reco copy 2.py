    ...
    def run(self):
        ...
        if left_file and right_file:
            ...
            if self.left_df is not None and self.right_df is not None:
                ...
                if submitted:
                    if st.session_state.reconciliation_type == "Reconcile by Matching Sum" and (len(left_cols) != len(right_cols) or len(left_cols) == 0):
                        st.error("For 'Reconcile by Matching Sum', number of key columns must be the same for both datasets and greater than zero.")
                    elif st.session_state.reconciliation_type == "Reconcile Line by Line" and len(left_cols) != len(right_cols):
