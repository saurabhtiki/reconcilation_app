...
class DataReconciliationApp:
    ...
    def reconcile(self, left_cols: list, right_cols: list, left_amount_col: str, right_amount_col: str, special_chars: str):
        ...
        left_indices = left_df_cleaned[left_df_cleaned['combined_key'].isin(reconciled_keys)].index
        right_indices = right_df_cleaned[right_df_cleaned['combined_key'].isin(reconciled_keys)].index
    ...
    def display_results(self):
        ...
        with tab1:
            ...
            with col1:
                ...
                reconciled_df_left = st.session_state.reconciled_left_df[st.session_state.reconciled_left_df['reconciliation_status'] != 'unreconciled']
                st.dataframe(reconciled_df_left)
            with col2:
                st.write(f":green[Reconciled records from {st.session_state.right_name}]")
                reconciled_df_right = st.session_state.reconciled_right_df[st.session_state.reconciled_right_df['reconciliation_status'] != 'unreconciled']
                st.dataframe(reconciled_df_right)
        ...
        with tab2:
            ...
            with col1:
                ...
                st.dataframe(st.session_state.reconciled_left_df[st.session_state.reconciled_left_df['reconciliation_status'] == 'unreconciled'])
            with col2:
                st.write(f":red[Unreconciled records from {st.session_state.right_name}]")
                st.dataframe(st.session_state.reconciled_right_df[st.session_state.reconciled_right_df['reconciliation_status'] == 'unreconciled'])
        ...
        with tab3:
            ...
            def manual_reconciliation_fragment():
                ...
                with col1:
                    ...
                    st.dataframe(unreconciled_left, key="left_manual_selection", on_select="rerun", selection_mode="multi-row")

                with col2:
                    st.write(f"Unreconciled records from {st.session_state.right_name}")
                    st.dataframe(unreconciled_right, key="right_manual_selection", on_select="rerun", selection_mode="multi-row")
                ...
                if left_selection_rows:
                    selected_left_rows = unreconciled_left.iloc[left_selection_rows]
                    left_sum = selected_left_rows[left_amount_col].sum()
                    metric_col1.metric("Selected Left Sum", f"{left_sum:,.2f}")

                right_sum = 0
                if right_selection_rows:
                    selected_right_rows = unreconciled_right.iloc[right_selection_rows]
                    right_sum = selected_right_rows[right_amount_col].sum()
                    metric_col2.metric("Selected Right Sum", f"{right_sum:,.2f}")

                if left_selection_rows and right_selection_rows:
                    metric_col3.metric("Difference", f"{left_sum - right_sum:,.2f}")
