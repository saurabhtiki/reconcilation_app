...
class DataReconciliationApp:
    ...
    @staticmethod
    @st.cache_data
    def clean_key(df: pd.DataFrame, columns: list, items_to_ignore: str, synonyms: dict | None = None) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            synonyms (dict): A dictionary of synonyms to replace.
        ...
        if synonyms:
            for col in columns:
                # Ensure column is of string type for replacement
                df_copy[col] = df_copy[col].astype(str)
                for original, synonym in synonyms.items():
                    df_copy[col] = df_copy[col].str.replace(original, synonym, regex=False)
    ...
    def synonym_dialog_content(self):
        ...
        def add_synonym():
            st.session_state.synonyms.append({"original": "", "synonym": ""})

        def remove_synonym(index):
            st.session_state.synonyms.pop(index)

        for i, synonym_pair in enumerate(st.session_state.synonyms):
            col1, col2, col3 = st.columns([3, 3, 1])
            with col1:
                st.session_state.synonyms[i]["original"] = st.text_input("Original", value=synonym_pair["original"], key=f"syn_orig_{i}")
            with col2:
                st.session_state.synonyms[i]["synonym"] = st.text_input("Synonym", value=synonym_pair["synonym"], key=f"syn_syn_{i}")
            with col3:
                st.button("➖", key=f"remove_syn_{i}", on_click=remove_synonym, args=(i,))

        st.button("Add Synonym", on_click=add_synonym)

        st.subheader("Save/Load Synonyms")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Save Synonyms to JSON"):
                synonym_dict = {item['original']: item['synonym'] for item in st.session_state.synonyms if item['original']}
                if synonym_dict:
                    with open("synonyms.json", "w") as f:
                        json.dump(synonym_dict, f, indent=4)
                    st.success("Synonyms saved to synonyms.json")

        with col2:
            uploaded_synonyms = st.file_uploader("Load Synonyms from JSON", type=['json'])
            if uploaded_synonyms:
                synonym_data = json.load(uploaded_synonyms)
                st.session_state.synonyms = [{"original": k, "synonym": v} for k, v in synonym_data.items()]
                st.rerun()

        if st.button("Close"):
            st.session_state.show_synonym_dialog = False
            st.rerun()
    ...
    def run(self):
        ...
        if left_file and right_file:
            ...
            if self.left_df is not None and self.right_df is not None:
                ...
                if st.button("Manage Synonyms 🔁"):
                    st.session_state.show_synonym_dialog = True

                if st.session_state.get("show_synonym_dialog"):
                    st.dialog("Synonym Mapping", self.synonym_dialog_content)
        ...
        if 'show_synonym_dialog' not in st.session_state:
            st.session_state.show_synonym_dialog = False
        ...
        if left_file and right_file:
            ...
            if self.left_df is not None and self.right_df is not None:
                ...
                if not st.session_state.get("show_synonym_dialog"):
                    with st.form("reconciliation_form"):
                        tab1, tab2, tab3 = st.tabs(["Column Mapping", "Amount Column Selection", "Characters/Phrases to Ignore"])
                ...
                if not st.session_state.get("show_synonym_dialog") and submitted:
    ...
    def reconcile(self, left_cols: list, right_cols: list, left_amount_col: str, right_amount_col: str, special_chars: str):
        ...
        """
        synonyms = {item['original']: item['synonym'] for item in st.session_state.get('synonyms', []) if item['original']}
        left_df_cleaned = self.clean_key(self.left_df, left_cols, special_chars, synonyms)
        right_df_cleaned = self.clean_key(self.right_df, right_cols, special_chars, synonyms)
